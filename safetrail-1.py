...
# --- AI Itinerary Planner (LangGraph + Groq-hosted LLM) ---
# Replaces the earlier placeholder itinerary text with a real LLM-backed workflow.
...
_itinerary_prompt = ChatPromptTemplate.from_messages([
    ("system", "You are a helpful travel assistant. Create a day trip itinerary for {city} "
               "based on the user's interests: {interests}. Provide a brief, bulleted itinerary."),
    ("human", "Create an itinerary for my day trip."),
])

def _get_llm():
    """Fetch the Groq API key from Streamlit secrets or the environment. Never hardcode keys in source."""
    api_key = None
    try:
...
def _validate_input_node(state: ItineraryState) -> ItineraryState:
    city = state.get("city", "").strip()
    interests = [i.strip() for i in state.get("interests", []) if i.strip()]
    if not city or not interests:
        return {**state, "error": "Please provide both a city and at least one interest."}
    return {**state, "city": city, "interests": interests, "error": ""}

def _generate_itinerary_node(state: ItineraryState) -> ItineraryState:
    if state.get("error"):
        return state
    llm = _get_llm()
    if llm is None:
        return {**state, "error": "AI planner is not configured (missing GROQ_API_KEY)."}
    response = llm.invoke(_itinerary_prompt.format_messages(
        city=state["city"], interests=", ".join(state["interests"])
    ))
    return {**state, "itinerary": response.content}

_itinerary_workflow = StateGraph(ItineraryState)
_itinerary_workflow.add_node("validate_input", _validate_input_node)
_itinerary_workflow.add_node("generate_itinerary", _generate_itinerary_node)
_itinerary_workflow.set_entry_point("validate_input")
_itinerary_workflow.add_edge("validate_input", "generate_itinerary")
_itinerary_workflow.add_edge("generate_itinerary", END)
_itinerary_app = _itinerary_workflow.compile()

def generate_ai_itinerary(city: str, interests_raw: str) -> str:
    """Run the LangGraph itinerary workflow and return itinerary text (or a user-facing error)."""
    interests = [i.strip() for i in interests_raw.split(",") if i.strip()]
    result = _itinerary_app.invoke({"city": city, "interests": interests, "itinerary": "", "error": ""})
    if result.get("error"):
            ...
            if st.form_submit_button("Generate Itinerary"):
                if city and interests:
                    with st.spinner("Creating your perfect itinerary..."):
                        itinerary = generate_ai_itinerary(city, interests)
                        st.markdown(itinerary)
                        if not itinerary.startswith("⚠️"):
